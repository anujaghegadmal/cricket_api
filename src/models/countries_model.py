...
class countries_model:
    ...
    def update_country_model(self,data,id):
        try:
            self.cursor.execute("UPDATE countries SET country='"+data["country"]+"', flag='"+data["flag"]+"' WHERE id="+str(id))
            return make_response("Country Updated Successfully",200)
        
        except Exception as e:
            make_response({"Error":str(e)},500)
            
    def delete_country_model(self,id):
        try:
